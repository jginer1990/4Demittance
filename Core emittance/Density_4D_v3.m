...
for i=1:numel(x)
    ...
    d=abs(Y-y(i)); minval=min(d(:));
    aux2=zeros(size(d)); aux2(d==minval)=1/dY; %to do: check what happens when minval is too large
    aux3=1./(2*pi.*sxp(i).*syp(i).*sqrt(1-rho(i).^2)).* ...
                    exp(-1./(2*(1-rho(i).^2)).*( ...
                        (Xp-xp(i)).^2./(sxp(i).^2)+(Yp-yp(i)).^2./(syp(i).^2) ...
                        -2*rho(i).*(Xp-xp(i)).*(Yp-yp(i))./(sxp(i).*syp(i)) ...
                        ) );
    Int = Int + int(i).*aux1.*aux2.*aux3;
    display([num2str(i/numel(x)*100) '%'])
end

%variant 2: consider uniform distribution as gaussian of sigma^2=dx^2/12
% Int=zeros(numel(X));
% for i=1:numel(x)
%     mu=[x(i); xp(i); y(i); yp(i)];
%     Sigma=[dX^2/12   0        0       0; ...
%              0     sx(i)^2    0     sxy(i); ...
%              0       0     dY^2/12     0; ...
%              0     sxy(i)     0     sy(i)^2 ] ;
%     rr=[X(:)'; Xp(:)'; Y(:)'; Yp(:)']-mu;
%     for m=1:size(rr,2)
%         exponent(m)= -1/2*rr(:,m)'*inv(Sigma)*rr(:,m) ;
%     end
%     Int = Int + 1/sqrt((2*pi)^4*det(Sigma))*exp(exponent);
% end
