...
X_vec= linspace(lb(1),ub(1),Npoints(1)); dx= mean(mean(diff(x,1,2))); dX=X_vec(2)-X_vec(1);
Xp_vec= linspace(lb(2),ub(2),Npoints(2));
Y_vec= linspace(lb(3),ub(3),Npoints(3)); dy= mean(mean(diff(y,1,1))); dY=X_vec(2)-X_vec(1);
...
for i=1:numel(x)
    d=abs(X(:,:,1,1)-x(i)); minval=min(d(:));
    aux1=zeros(size(d)); aux1(d==minval)=1/dX; %to do: check what happens when minval is too large
    aux2=1/sqrt(2*pi)./sxp(i).*exp(-(Xp(:,:,1,1)-xp(i)).^2./(2*sxp(i).^2));
    fxxp = fxxp + int(i).*aux1.*aux2;

    d=abs(X(:,1,:,1)-x(i)); minval=min(d(:));
    aux1=zeros(size(d)); aux1(d==minval)=1/dX; %to do: check what happens when minval is too large
    d=abs(Y(:,1,:,1)-y(i)); minval=min(d(:));
    aux2=zeros(size(d)); aux2(d==minval)=1/dY; %to do: check what happens when minval is too large
    fxy = fxy + int(i).*aux1.*aux2;

    d=abs(X(:,1,1,:)-x(i)); minval=min(d(:));
    aux1=zeros(size(d)); aux1(d==minval)=1/dX; %to do: check what happens when minval is too large
    aux2=1/sqrt(2*pi)./syp(i).*exp(-(Yp(:,1,1,:)-yp(i)).^2./(2*syp(i).^2));
    fxyp = fxyp + int(i).*aux1.*aux2;
    
    d=abs(Y(1,:,:,1)-y(i)); minval=min(d(:));
    aux1=zeros(size(d)); aux1(d==minval)=1/dY; %to do: check what happens when minval is too large
    aux2=1/sqrt(2*pi)./sxp(i).*exp(-(Xp(1,:,:,1)-xp(i)).^2./(2*sxp(i).^2));
    fxpy = fxpy + int(i).*aux1.*aux2;
    
    aux1=1./(2*pi.*sxp(i).*syp(i).*sqrt(1-rho(i).^2)).* ...
                    exp(-1./(2*(1-rho(i).^2)).*( ...
                        (Xp(1,:,1,:)-xp(i)).^2./(sxp(i).^2)+(Yp(1,:,1,:)-yp(i)).^2./(syp(i).^2) ...
                        -2*rho(i).*(Xp(1,:,1,:)-xp(i)).*(Yp(1,:,1,:)-yp(i))./(sxp(i).*syp(i)) ...
                        ) );
    fxpyp = fxpyp + int(i).*aux1;
    
    d=abs(Y(1,1,:,:)-y(i)); minval=min(d(:));
    aux1=zeros(size(d)); aux1(d==minval)=1/dY; %to do: check what happens when minval is too large
    aux2=1/sqrt(2*pi)./syp(i).*exp(-(Yp(1,1,:,:)-yp(i)).^2./(2*syp(i).^2));
    fyyp = fyyp + int(i).*aux1.*aux2;
end
