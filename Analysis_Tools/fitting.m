function [sigma,x0,intx] = fitting(x,y,barwidth,driftLength,analysis)
...
modelFunerf = @(p,x) p(5)+p(4)*x+ p(3).*(-erf((x-p(1)+ barwidth/2)/(sqrt(2)*driftLength*p(2)^2))+erf((x-p(1)-barwidth/2)/(sqrt(2)*driftLength*p(2)^2)));
