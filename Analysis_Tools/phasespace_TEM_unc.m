...
for j=1:length(locsy)-1; % Loop over y
    ...
    for i=1:length(locsx); % Loop over x
        ...
        if flag == 0
            ...
            try
                [sx(j,i), xb(j,i), intx(j,i), covx{j,i}] = fittingtest_unc(regionx, Ixroi, barwidth_x_pix, mask_prop.driftLength,analysis); % Fit erf of region between two peaks
            catch
...
for j=1:length(locsx)-1; % Loop over x
    ...
    for i=1:length(locsy); % Loop over y
        ...
        if flag == 0
            ...
            try
                [sy(i,j), yb(i,j), inty(i,j), covy{i,j}] = fittingtest_unc(regiony, Iyroi', barwidth_y_pix, mask_prop.driftLength,analysis); % Fit erf of region between two peaks.
            catch
